...
## v0.6.7（2026-07-29）全应用 UI 重设计

本轮修复 v0.6.6 的 14 个问题 + 5 项交接文档遗漏，核心架构从"全透明玻璃"转向"亚克力侧边栏+实体内容区"。Debug + Release 双构建通过。

### 第一层：核心架构转变（F11-1~3）

- **[F11-1]** 操作区实体背景（日间#F5F5F5/夜间#2D2D2D），侧边栏保留亚克力（问题4/9）
- **[F11-2]** 子窗口全部实体背景+移除 FFGlassView（问题5/6）
- **[F11-3]** 主题恢复三态（light/dark/system）+ 全局 ErrorBoundary（遗漏B4/A3）

### 第二层：主窗口重设计（F11-4~6）

- **[F11-4]** 路径栏完全仿访达重写（固定位置+每段可点击+分隔符弹同级+高亮，问题12）
- **[F11-5]** 分组重新设计仿访达（粘性header+计数徽章+缩进+修复Title重叠，问题14）
- **[F11-6]** 详情栏空白占位图标+设备栏实体背景（问题10/4）

### 第三层：功能修复（F11-7~9）

- **[F11-7]** 卡顿修复-缩略图异步+主线程优化（NSWorkspace.icon 缓存+磁盘缓存异步，问题7）
- **[F11-8]** 搜索修复(子目录递归+allFiles)+标签点击筛选当前面板（问题11/3）
- **[F11-9]** 复制/移动底部进度栏（问题1）

### 第四层：遗漏项补充（F11-10~11）

- **[F11-10]** 标签系统与 macOS 原生标签同步（_kMDItemUserTags 双向读写，遗漏8.5）
- **[F11-11]** 工具面板展开+大目录分页性能（首批500+异步追加，遗漏C1/C5）

### 明确废弃的约束

- A6 侧边栏区段可折叠：保持不折叠（用户确认）
- B1 独立标签列：保持 4 列+内联药丸
- C3 Intel 通用二进制：留下一轮

---

## v0.6.6（2026-07-28）仿访达完整重设计

本轮修复 v0.6.5 用户反馈的 20 个问题，完全克隆访达视觉与交互。Debug + Release 双构建通过。

### 第一层：窗口架构重建（F10-1~3）

- **[F10-1]** 移除 title 栏占位，红绿灯浮在侧边栏上方 + window.delegate 设置（问题9/15辅助）
- **[F10-2]** 选中渲染修复 - 移除 F7 错误的 0.6alpha 背景，改回 .clear（问题14，修正 F7 错误）
- **[F10-3]** 设备区域浮动到窗口左下角独立区域，展开显示所有设备不需滚动（问题18）

### 第二层：侧边栏重构（F10-4~5）

- **[F10-4]** 收藏夹改回彩色真实图标 + 不折叠 + 标签在下方（问题1，修正 F4 错误）
- **[F10-5]** 标签模块重设计 - 圆点+完整标签名垂直排列，不遮挡标题（问题2）

### 第三层：文件列表重构（F10-6~8）

- **[F10-6]** 列宽自动调整占满操作区（问题4）
- **[F10-7]** 药丸列头 + 可拖动调整顺序 + 列头点击升降序，移除独立升降序按钮（问题5/12）
- **[F10-8]** 分组功能照搬访达 - 按种类/日期/大小，section header 渲染（问题6）

### 第四层：网格视图重构（F10-9）

- **[F10-9]** 网格视图照搬访达选中样式 + 缩略图修复（NSImage.size bug + representationTypes + grid 复用校验 + appearance 同步）（问题7/10，修正 F7/F8 遗漏）

### 第五层：功能接线修复（F10-10）

- **[F10-10]** 设置窗口 OpenSettings 通知接线 + 搜索实时生效 + 新增"在另一侧打开" + 右键菜单日志 + 查重/重命名补实现（问题3/11/13/15/16/17）

### 第六层：视觉打磨（F10-11）

- **[F10-11]** 整体视觉 + 夜间模式克隆访达 - 详情栏材质统一 .sidebar、列头字号 11pt、收藏夹行高 28pt（问题19/20）
